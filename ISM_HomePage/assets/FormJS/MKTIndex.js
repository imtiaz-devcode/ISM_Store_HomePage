...
function SavePromotionalVideo() {
    ...
    postRequest_File(BaseURL + "ISM_HomePage_WebService.asmx/SavePromotionalVideo", formData, function (res) {
...
function SavePromotionalPDF() {
    ...
    postRequest_File(BaseURL + "ISM_HomePage_WebService.asmx/SavePromotionalPDF", formData, function (res) {
        var xmlString = new XMLSerializer().serializeToString(res);
        var $xml = $($.parseXML(xmlString));
        var Message = $($xml[0].all[1]).text();
        var Status = $($xml[0].all[2]).text();
